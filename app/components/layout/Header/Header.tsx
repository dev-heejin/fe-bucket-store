...
export default function Header() {
  return (
    ...
    <header className="w-full flex flex-col z-50">
      {/* 검정 배경 영역 고정 */}
      <div className="w-full p-[24px] hidden lg:fixed lg:top-0 lg:left-0 lg:flex lg:bg-black lg:items-center z-50">
        <div className="w-full max-w-[1920px] flex items-center px-[24px] mx-auto">
          <h1 className="font-bold text-4xl text-white mr-[16px]">BUCKET</h1>
          <div className="flex gap-[16px] text-md font-bold text-white [&_*:first-of-type]:text-red-500">
            {GNB_CATEGORY.map((x) => (
              <span key={x}>{x}</span>
            ))}{' '}
            |{' '}
      ...
      {/* '티셔츠' 텍스트는 기본적으로 노출되고, 스크롤 시에만 가려짐 */}
      <div
        className="w-full py-[20px] flex justify-center lg:justify-start items-center lg:mt-[80px] lg:p-[40px_20px] relative z-10">
        <div className="w-full max-w-[1920px] flex items-center px-[24px] lg:mx-auto">
          <div className="flex justify-start items-center gap-[12px]">
            <ArrowBack
              className={'lg:!hidden'}
              sx={{ color: 'gray', fontSize: 24 }}
          ...
          <div className="flex gap-[16px] justify-end items-center ml-auto lg:!hidden">
            <SearchIcon sx={{ color: 'gray', fontSize: 24 }} />
            <LocalMall sx={{ color: 'gray', fontSize: 24 }} />
          </div>
        </div>
      </div>
    </header>
